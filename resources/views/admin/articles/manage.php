                                    ...
                                    <tr class="fade-in" style="animation-delay: <?= $k * 0.1 ?>s;">
                                        <td data-label="Article">
                                            <div class="d-flex align-items-center gap-3">
                                                ...
                                                <div>
                                                    <div class="fw-semibold text-white" title="{{{ $article->title }}}">{{{ getExcerpt($article->title, 50) }}}</div>
                                                    <small class="text-secondary">
                                                        <?php if ($article->tags): ?>
                                                            <i class="bi bi-tags me-1"></i>{{{ getExcerpt($article->tags, 30) }}} •
                                                        <?php endif; ?>
                                                        <i class="bi bi-file-text me-1"></i>{{{ getExcerpt($article->meta_title, 25) }}}
                                                    </small>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td data-label="Status">
                                            <div class="text-center">
                                                <span class="badge {{ $article->status == 'publish' ? 'bg-success' : 'bg-warning' }} text-capitalize">
                                                    <i class="bi bi-{{ $article->status == 'publish' ? 'check-circle' : 'file-earmark' }} me-1"></i>
                                        ...
                                        <td data-label="Actions" class="text-end">
                                            <div class="d-flex gap-2 justify-content-end">
                                                <a href="{{{ url("/admin/articles/view/{$article->slug}") }}}" class="btn btn-ghost action-btn" data-bs-toggle="tooltip" title="View Article Details">
                                                    <i class="bi bi-eye"></i>
                                                </a>
                                                <a href="{{{ url("/admin/articles/edit/{$article->slug}") }}}" class="btn btn-outline-warning action-btn" data-bs-toggle="tooltip" title="Edit Article">
                                                    <i class="bi bi-pencil"></i>
                                                </a>
                                                <?php if ($article->status === 'publish'): ?>
                                                    <a href="{{{ url("/articles/{$article->slug}") }}}" class="btn btn-outline-info action-btn" data-bs-toggle="tooltip" title="View Published Article" target="_blank">
